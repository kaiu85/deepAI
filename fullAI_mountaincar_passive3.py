...
for i in range(n_steps):
    ...
    if i == 0:
        with open("log_fullAI_mountaincar_passive3.txt", "w") as myfile:
            myfile.write("%f %f %f %f %f %f %f\n" % (oFE_mean, oKL_st_mean, op_ot_mean, op_oht_mean, po_oat_mean, ofixed_prior_cost, FE_min))
    else:
        with open("log_fullAI_mountaincar_passive3.txt", "a") as myfile:
            myfile.write("%f %f %f %f %f %f %f\n" % (oFE_mean, oKL_st_mean, op_ot_mean, op_oht_mean, po_oat_mean, ofixed_prior_cost, FE_min))
    ...
    if i % 100 == 0:
        with open('fullAI_mountaincar_passive3.pkl', 'w') as f:
            cPickle.dump(params, f)
            
    # Save best parameters
    if oFE_mean < FE_min:
        FE_min = oFE_mean
        with open('fullAI_mountaincar_passive3_best.pkl', 'w') as f:
            cPickle.dump(params, f)
