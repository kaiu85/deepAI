...
# Load learned Agent
with open('fullAI_mountaincar_passive3_best.pkl', 'r') as f:#open('fullAI_mountaincar_state_prior_scaled_explicit_prior8passive2_best.pkl', 'r') as f:
...
pos_t0 = theano.shared(name = 'pos_t0', value = 0.5*numpy.random.randn( n_o, n_proc ).astype( dtype = theano.config.floatX ), borrow = True )
...
ots_given = theano.shared(name = 's_t0', value = -2.0*numpy.ones( (100, n_o,n_samples) ).astype( dtype = theano.config.floatX ), borrow = True )
