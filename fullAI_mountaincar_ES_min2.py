...
'''
Wq_hst3_hst2 = theano.shared(
    value=initweight(n_s, n_s),
    name='Wq_hst3_hst2',
    borrow=True
)

params.append(Wq_hst3_hst2)

bq_hst3 = theano.shared(
    value=initconst(n_s, 1),
    name='bq_hst3',
    borrow=True,
    broadcastable=(False,True)
...
params.append(bq_hst3)
'''
Wq_stmu_hst3 = theano.shared(
    value=initweight(n_s, n_s),
    name='Wq_stmu_hst3',
    borrow=True
)

params.append(Wq_stmu_hst3)
...
Wq_stsig_hst3 = theano.shared(
    value=initweight(n_s, n_s),
    name='Wq_stsig_hst3',
    borrow=True
)

params.append(Wq_stsig_hst3)
...
def inner_fn(t, stm1, oat, ot, oht, pos, vt):
    ...
    return st, oat_new, ot_new, oht_new, pos_new, vt_new, FEt, KL_st, stmu, stsig, force, p_ot, p_oht, p_oat, prior_stmu, prior_stsig
...
((states_th, oat_th, ot_th, oht_th, post_th, vt_th, FEt_th, KL_st_th, stmu_th, stsig_th, aht_th, p_ot_th, p_oht_th, p_oat_th, prior_stmu_th, prior_stsig_th), fe_updates) =\
...
run_agent_scan = theano.function(inputs = [], outputs = [states_th, oat_th, ot_th, oht_th, FEt_th, KL_st_th, stmu_th, stsig_th, aht_th, KL_st_mean], allow_input_downcast = True, on_unused_input='ignore')
...
states, actions, observations, rewards, FEs, KLs, stmus, stsigs, ahts, fp_cost = run_agent_scan()
...
train = theano.function(
        inputs=[],
        outputs=[FE_mean, KL_st_mean, p_ot_mean, p_oht_mean, p_oat_mean], 
        updates=updates,
...
for i in range(n_steps):    
    ...
    [oFE_mean, oKL_st_mean, op_ot_mean, op_oht_mean, po_oat_mean] = train()
    
    print [oFE_mean, oKL_st_mean, op_ot_mean, op_oht_mean, po_oat_mean]
       
    if i == 0:
        with open("log_fullAI_mountaincar_ES_min2.txt", "w") as myfile:
            myfile.write("%f %f %f %f %f %f\n" % (oFE_mean, oKL_st_mean, op_ot_mean, op_oht_mean, po_oat_mean, FE_min))
    else:
        with open("log_fullAI_mountaincar_ES_min2.txt", "a") as myfile:
            myfile.write("%f %f %f %f %f %f\n" % (oFE_mean, oKL_st_mean, op_ot_mean, op_oht_mean, po_oat_mean, FE_min))
    ...
    if i % 100 == 0:
        with open('fullAI_mountaincar_ES_min2.pkl', 'w') as f:
            cPickle.dump(params, f)
            
    # Save best parameters
    if oFE_mean < FE_min:
        FE_min = oFE_mean
        with open('fullAI_mountaincar_ES_min2_best.pkl', 'w') as f:
            cPickle.dump(params, f)
