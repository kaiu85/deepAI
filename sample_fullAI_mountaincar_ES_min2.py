...
def initweightXavier(shape1, shape2, scale = 1.0):
...
# Load learned Agent
with open('fullAI_mountaincar_ES_min2_best.pkl', 'r') as f:
    params = cPickle.load(f)

[Wq_hst_ot, Wq_hst_oht, Wq_hst_oat, Wq_hst_stm1, bq_hst,\
Wq_hst2_hst, bq_hst2,\
Wq_stmu_hst3, bq_stmu,\
Wq_stsig_hst3, bq_stsig,\
...
def inner_fn(t, stm1, oat, ot, oht, pos, vt):
    ...
    #hst3 =  T.nnet.relu( T.dot(Wq_hst3_hst2,hst2) + bq_hst3 )

    stmu =  T.tanh( T.dot(Wq_stmu_hst3,hst2) + bq_stmu )
    stsig = T.nnet.softplus( T.dot(Wq_stsig_hst3,hst2) + bq_stsig ) + sig_min_states
    ...
    oat_new = T.tanh(0.0*oat + 1.0*st[1,:])
...
def encode(ot, oht, oat, stm1):
    ...
    #hst3 =  T.tanh( T.dot(Wq_hst3_hst2,hst2) + bq_hst3 )

    stmu =  T.tanh( T.dot(Wq_stmu_hst3,hst) + bq_stmu )
    stsig = T.nnet.softplus( T.dot(Wq_stsig_hst3,hst) + bq_stsig ) + sig_min_states
    
    # Rescale representation to fit within linear response of the tanh-nonlinearity
    stmu = T.set_subtensor(stmu[0,:],0.1*ot[0,:])
    stsig = T.set_subtensor(stsig[0,:],1e-3)
...
def init_trajectory(ot_given, stm1):
    ...
    ((st, ot, oht, oat), _) = theano.scan(fn=inner_fn_init_trajectory, 
...
ots_given = theano.shared(name = 's_t0', value = -0.5*numpy.ones( (10, n_oa,n_samples) ).astype( dtype = theano.config.floatX ), borrow = True )
...
((states_sampled_it, observations_sampled_it, rewards_sampled_it, actions_observations_sampled_it), _) =\
                     ...
                     outputs_info=[s_t0_sample_actions_given, None, None, None],
                     sequences = [ots_given]
                     )  
                     
s_t0_it = states_sampled_it[-1]

st0_init = theano.shared(name = 's_t0', value = 0.0*numpy.random.randn( n_s,n_samples ).astype( dtype = theano.config.floatX ), borrow = True )
ot0_init = theano.shared(name = 's_t0', value = -0.5 + 0.0*numpy.random.randn( n_o,n_samples ).astype( dtype = theano.config.floatX ), borrow = True )
oht0_init = theano.shared(name = 's_t0', value = 0.0*numpy.random.randn( n_oh,n_samples ).astype( dtype = theano.config.floatX ), borrow = True )
oat0_init = theano.shared(name = 's_t0', value = 0.0*numpy.random.randn( n_oa,n_samples ).astype( dtype = theano.config.floatX ), borrow = True )

st0_init2 = encode(ot0_init, oht0_init, oat0_init, st0_init)
st0_init3 = encode(ot0_init, oht0_init, oat0_init, st0_init2)

oats_given = theano.shared(name = 's_t0', value = numpy.concatenate( (-0.0*numpy.ones( (10, n_oa,n_samples) ), 0.0*numpy.ones( (n_sample_steps - 10, n_oa,n_samples) )), axis = 0).astype( dtype = theano.config.floatX ), borrow = True )
...
((states_sampled_ag, observations_sampled_ag, rewards_sampled_ag, actions_observations_sampled_ag), _) =\
                     ...
                     outputs_info=[st0_init3, None, None, None],
