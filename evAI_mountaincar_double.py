...
for i in range(n_steps):
    ...
    if i == 0:
        with open("log_evAI_mountaincar_double.txt", "w") as myfile:
            myfile.write("%f %f %f\n" % (oFE_mean, FE_min, ograd_corr))
    else:
        with open("log_evAI_mountaincar_double.txt", "a") as myfile:
            myfile.write("%f %f %f\n" % (oFE_mean, FE_min, ograd_corr))
    ...
    if i % 100 == 0:
        with open('evAI_mountaincar_double.pkl', 'w') as f:
            cPickle.dump(params, f)
            
    # Save best parameters
    if oFE_mean < FE_min:
        FE_min = oFE_mean
        with open('evAI_mountaincar_double_best.pkl', 'w') as f:
            cPickle.dump(params, f)
